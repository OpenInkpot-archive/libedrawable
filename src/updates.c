...
DrawableUpdate        *
__drawable_MergeUpdate(DrawableUpdate * u, int w, int h, int hgapmax)
{
   DrawableUpdate        *nu = NULL, *uu;
   struct _tile       *t;
   int                 tw, th, x, y, i;
   int                *gaps = NULL;
             ...
             if (T(x, y).used & T_USED)
               {
                  ...
                  nu = __drawable_AddUpdate(nu, (x << TB), (y << TB),
                                         (ww << TB), (hh << TB));
               }
          }
     }
...
DrawableUpdate        *
__drawable_AddUpdate(DrawableUpdate * u, int x, int y, int w, int h)
{
   DrawableUpdate        *nu;

   if ((w < 1) || (h < 1) || ((x + w) < 1) || ((y + h) < 1))
      return u;
   nu = malloc(sizeof(DrawableUpdate));
   nu->x = x;
   nu->y = y;
   nu->w = w;
   nu->h = h;
...
void
__drawable_FreeUpdates(DrawableUpdate * u)
{
   DrawableUpdate        *uu;
...
{
   DrawableUpdate        *uu, *cu, *pu, *ru;

   if (!u)
      return NULL;

   uu = malloc(sizeof(DrawableUpdate));
   memcpy(uu, u, sizeof(DrawableUpdate));
   cu = u->next;
   pu = u;
   ru = uu;
   while (cu)

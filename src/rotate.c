...
static void
__drawable_RotateSampleInside(DATA32 * src, DATA32 * dest, int sow, int dow,
...
static void
__drawable_RotateAAInside(DATA32 * src, DATA32 * dest, int sow, int dow,
                       int dw, int dh, int x, int y,
                       int dxh, int dyh, int dxv, int dyv)
{
   int                 i;
...
void
__drawable_RotateSample(DATA32 * src, DATA32 * dest, int sow, int sw, int sh,
                     ...
                     int dxh, int dyh, int dxv, int dyv)
{
   ...
   if (__check_inside_coords(x, y, dxh, dyh, dxv, dyv, dw, dh, sw, sh))
     {
        __drawable_RotateSampleInside(src, dest, sow, dow, dw, dh, x, y,
                                   dxh, dyh, dxv, dyv);
        return;
...
\*/
void
__drawable_RotateAA(DATA32 * src, DATA32 * dest, int sow, int sw, int sh,
                 int dow, int dw, int dh, int x, int y,
                 int dxh, int dyh, int dxv, int dyv)
{
   int                 i;
   ...
   if (__check_inside_coords(x, y, dxh, dyh, dxv, dyv, dw, dh, sw - 1, sh - 1))
     {
        __drawable_RotateAAInside(src, dest, sow, dow, dw, dh, x, y,
                               dxh, dyh, dxv, dyv);
        return;
...
void
__drawable_BlendImageToImageSkewed(DrawableImage * im_src, DrawableImage * im_dst,
...
#ifdef DO_MMX_ASM
             if (do_mmx)
                __drawable_mmx_RotateAA(src, data, im_src->w, ssw, ssh, w, w, h,
                                     x, y, dxh, dyh, dxv, dyv);
             else
#endif
                __drawable_RotateAA(src, data, im_src->w, ssw, ssh, w, w, h,
                                 x, y, dxh, dyh, dxv, dyv);

          }
        else
          {
             __drawable_RotateSample(src, data, im_src->w, ssw, ssh, w, w, h,
                                  x, y, dxh, dyh, dxv, dyv);

          }
        __drawable_BlendRGBAToData(data, w, h, im_dst->data,

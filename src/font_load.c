...
/* separate fontname and size, find font file, start drawable_font_load() then */
DrawableFont          *
drawable_font_load_joined(const char *fontname)
{
   ...
   if (!file)
      return NULL;
   fn = drawable_font_load(file, faceidx, size);
   free(file);
   return fn;
}
...
drawable_font_load(const char *name, int faceidx, int size)
{
   ...
   fonts = drawable_object_list_prepend(fonts, fn);
   return fn;
}

void
drawable_font_free(DrawableFont * fn)
{
   fn->references--;
   if (fn->references == 0)
     {
        drawable_font_modify_cache_by(fn, 1);
        drawable_font_flush();
     }
}

int
drawable_font_insert_into_fallback_chain_imp(DrawableFont * fn, DrawableFont * fallback)
{
   ...
   drawable_font_remove_from_fallback_chain_imp(fallback);

   /* insert fallback into fn's font chain */
   DrawableFont          *tmp = fn->fallback_next;
...
void
drawable_font_remove_from_fallback_chain_imp(DrawableFont * fn)
{
...
static int
font_modify_cache_cb(Drawable_Hash * hash, const char *key, void *data,
                     void *fdata)
{
   int                *dir;
   Drawable_Font_Glyph   *fg;

   fg = data;
   dir = fdata;
   font_cache_usage += (*dir) * ((fg->glyph_out->bitmap.width * fg->glyph_out->bitmap.rows) + sizeof(Drawable_Font_Glyph) + sizeof(Drawable_Object_List) + 400);      /* fudge values */
   return 1;
   hash = 0;
   key = 0;
}
...
void
drawable_font_modify_cache_by(DrawableFont * fn, int dir)
{
   ...
   if (fn->glyphs)
      sz_hash = sizeof(Drawable_Hash);
   drawable_hash_foreach(fn->glyphs, font_modify_cache_cb, &dir);
   font_cache_usage += dir * (sizeof(DrawableFont) + sz_name + sz_file + sz_hash + sizeof(FT_FaceRec) + 16384);    /* fudge values */
}

int
drawable_font_cache_get(void)
{
   return font_cache;
}

void
drawable_font_cache_set(int size)
{
   font_cache = size;
   drawable_font_flush();
}
...
void
drawable_font_flush(void)
{
   if (font_cache_usage < font_cache)
      return;
   while (font_cache_usage > font_cache)
      drawable_font_flush_last();
}

static int
font_flush_free_glyph_cb(Drawable_Hash * hash, const char *key, void *data,
                         void *fdata)
{
   Drawable_Font_Glyph   *fg;
...
void
drawable_font_flush_last(void)
{
   Drawable_Object_List  *l;
   DrawableFont          *fn = NULL;

   for (l = fonts; l; l = l->next)
     {
        DrawableFont          *fn_tmp;

        fn_tmp = (DrawableFont *) l;
   ...
   fonts = drawable_object_list_remove(fonts, fn);
   drawable_font_modify_cache_by(fn, -1);

   drawable_hash_foreach(fn->glyphs, font_flush_free_glyph_cb, NULL);
   drawable_hash_free(fn->glyphs);
...
DrawableFont          *
drawable_font_find(const char *name, int size)
{
   Drawable_Object_List  *l;
...
/* font pathes */
void
drawable_font_add_font_path(const char *path)
{
   fpath_num++;
   if (!fpath)
...
void
drawable_font_del_font_path(const char *path)
{
   int                 i, j;
...
int
drawable_font_path_exists(const char *path)
{
   int                 i;
...
char              **
drawable_font_list_font_path(int *num_ret)
{
   *num_ret = fpath_num;
   return fpath;
}
...
/* fonts list */
char              **
drawable_font_list_fonts(int *num_ret)
{
   int                 i, j, d, l = 0;
   ...
   for (i = 0; i < fpath_num; i++)
     {
        dir = __drawable_FileDir(fpath[i], &d);
        if (dir)
          {
             for (j = 0; j < d; j++)
               {
                  ...
                  if ((p = strrchr(dir[j], '.')))
                     *p = '\0';
                  if (!__drawable_ItemInList(list, l, dir[j]))
                    {
                       if (__drawable_FileIsFile(path))
                         {
                            FT_Face             f;

...
{
   DrawablePointDrawFunction pfunc;
   ...
   pfunc = __drawable_GetPointDrawFunction(op, IMAGE_HAS_ALPHA(im), blend);
   if (pfunc)
      pfunc(color, im->data + (im->w * y) + x);
   if (make_updates)
      return __drawable_AddUpdate(NULL, x, y, 1, 1);
   return NULL;
}

static int
__drawable_SimpleLine_DrawToData(int x0, int y0, int x1, int y1, DATA32 color,
                              DATA32 * dst, int dstw, int clx, int cly, int clw,
                              int clh, int *cl_x0, int *cl_y0, int *cl_x1,
                              int *cl_y1, DrawableOp op, char dst_alpha,
                              char blend)
{
   DrawablePointDrawFunction pfunc;
   DrawableSpanDrawFunction sfunc;
   ...
   if (dy == 0)
     {
        sfunc = __drawable_GetSpanDrawFunction(op, dst_alpha, blend);
        if (!sfunc)
           return 0;
   ...
   pfunc = __drawable_GetPointDrawFunction(op, dst_alpha, blend);
   if (!pfunc)
      return 0;
...
static int
__drawable_Line_DrawToData(int x0, int y0, int x1, int y1, DATA32 color,
                        DATA32 * dst, int dstw, int clx, int cly, int clw,
                        ...
                        DrawableOp op, char dst_alpha, char blend)
{
   ...
   if ((dx == 0) || (dy == 0) || (dx == dy) || (dx == -dy))
      return __drawable_SimpleLine_DrawToData(x0, y0, x1, y1, color,
...
static int
__drawable_Line_DrawToData_AA(int x0, int y0, int x1, int y1, DATA32 color,
                           DATA32 * dst, int dstw, int clx, int cly, int clw,
                           int clh, int *cl_x0, int *cl_y0, int *cl_x1,
                           int *cl_y1, DrawableOp op, char dst_alpha, char blend)
{
   DrawablePointDrawFunction pfunc;
   int                 px, py, x, y, prev_x, prev_y;
                         ...
                         char make_updates)
{
   ...
   if ((x0 == x1) && (y0 == y1))
      return __drawable_Point_DrawToImage(x0, y0, color,
                                       im, clx, cly, clw, clh,
   ...
   if (anti_alias)
      drew = __drawable_Line_DrawToData_AA(x0, y0, x1, y1, color,
                                        im->data, im->w, clx, cly, clw, clh,
                                        &cl_x0, &cl_y0, &cl_x1, &cl_y1,
                                        op, IMAGE_HAS_ALPHA(im), blend);
   else
      drew = __drawable_Line_DrawToData(x0, y0, x1, y1, color,
                                     im->data, im->w, clx, cly, clw, clh,
                                     &cl_x0, &cl_y0, &cl_x1, &cl_y1,
                                     op, IMAGE_HAS_ALPHA(im), blend);
   ...
   if (drew && make_updates)
     {
        ...
        return __drawable_AddUpdate(NULL, cl_x0, cl_y0, w, h);
     }

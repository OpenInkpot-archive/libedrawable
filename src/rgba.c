...
void
__drawable_RGBASetupContext(Context * ct)
{
...
void
__drawable_RGBA_init(void *rd, void *gd, void *bd, int depth, DATA8 palette_type)
{
...
static void
__drawable_RGBA_to_RGB565_fast(DATA32 * src, int src_jump,
                            DATA8 * dst, int dow,
...
static void
__drawable_RGBA_to_RGB565_dither(DATA32 * src, int src_jump,
                              DATA8 * dst, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_BGR565_fast(DATA32 * src, int src_jump,
                            DATA8 * dst, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_BGR565_dither(DATA32 * src, int src_jump,
                              DATA8 * dst, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB555_fast(DATA32 * src, int src_jump,
                            DATA8 * dst, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB555_dither(DATA32 * src, int src_jump,
                              DATA8 * dst, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_BGR555_fast(DATA32 * src, int src_jump,
                            DATA8 * dst, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_BGR555_dither(DATA32 * src, int src_jump,
                              DATA8 * dst, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB332_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB332_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB666_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB666_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB232_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB232_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB222_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB222_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB221_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB221_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB121_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB121_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB111_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB111_dither(DATA32 * src, int src_jump,
                              DATA8 * dest, int dow,
                              int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB1_fast(DATA32 * src, int src_jump,
                          DATA8 * dest, int dow,
                          int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB1_dither(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_A1_fast(DATA32 * src, int src_jump,
                        DATA8 * dest, int dow,
...
static void
__drawable_RGBA_to_A1_dither(DATA32 * src, int src_jump,
                          DATA8 * dest, int dow,
                          int width, int height, int dx, int dy, int threshold)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB8888_fast(DATA32 * src, int src_jump,
                             DATA8 * dst, int dow,
                             int width, int height, int dx, int dy)
{
   int                 y, w, h;
...
static void
__drawable_RGBA_to_BGR8888_fast(DATA32 * src, int src_jump,
                             DATA8 * dst, int dow,
                             int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_RGB888_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_BGR888_fast(DATA32 * src, int src_jump,
                            DATA8 * dest, int dow,
                            int width, int height, int dx, int dy)
{
   int                 x, y, w, h;
...
static void
__drawable_RGBA_to_Nothing(DATA32 * src, int src_jump,
                        DATA8 * dest, int dow,
                        int width, int height, int dx, int dy)
{
   ...
   if (depth == 16)
        ...
        if (hiq)
          {
             if ((rm == 0xf800) && (gm == 0x7e0) && (bm == 0x1f))
                return __drawable_RGBA_to_RGB565_dither;
             if ((rm == 0x7c00) && (gm == 0x3e0) && (bm == 0x1f))
                return __drawable_RGBA_to_RGB555_dither;
             if ((bm == 0xf800) && (gm == 0x7e0) && (rm == 0x1f))
                return __drawable_RGBA_to_BGR565_dither;
             if ((bm == 0x7c00) && (gm == 0x3e0) && (rm == 0x1f))
                return __drawable_RGBA_to_BGR555_dither;
          }
        else
          {
#ifdef DO_MMX_ASM
             if (__drawable_get_cpuid() && CPUID_MMX)
               {
                  if ((rm == 0xf800) && (gm == 0x7e0) && (bm == 0x1f))
                     return __drawable_mmx_rgb565_fast;
                  if ((rm == 0x7c00) && (gm == 0x3e0) && (bm == 0x1f))
                     return __drawable_mmx_rgb555_fast;
                  if ((bm == 0xf800) && (gm == 0x7e0) && (rm == 0x1f))
                     return __drawable_mmx_bgr565_fast;
                  if ((bm == 0x7c00) && (gm == 0x3e0) && (rm == 0x1f))
                     return __drawable_mmx_bgr555_fast;
               }
             else
#endif
               {
                  if ((rm == 0xf800) && (gm == 0x7e0) && (bm == 0x1f))
                     return __drawable_RGBA_to_RGB565_fast;
                  if ((rm == 0x7c00) && (gm == 0x3e0) && (bm == 0x1f))
                     return __drawable_RGBA_to_RGB555_fast;
                  if ((bm == 0xf800) && (gm == 0x7e0) && (rm == 0x1f))
                     return __drawable_RGBA_to_BGR565_fast;
                  if ((bm == 0x7c00) && (gm == 0x3e0) && (rm == 0x1f))
                     return __drawable_RGBA_to_BGR555_fast;
               }
          }
        return NULL;
     }
   else if (depth == 32)
     {
        if ((rm == 0xff0000) && (gm == 0xff00) && (bm == 0xff))
           return __drawable_RGBA_to_RGB8888_fast;
        return NULL;
     }
   else if (depth == 24)
     {
        if ((rm == 0xff0000) && (gm == 0xff00) && (bm == 0xff))
           return __drawable_RGBA_to_RGB888_fast;
        return NULL;
     }
   else if (depth == 8)
     {
        if (hiq)
          {
             if (palette_type == 0)
                return __drawable_RGBA_to_RGB332_dither;
             if (palette_type == 1)
                return __drawable_RGBA_to_RGB232_dither;
             if (palette_type == 2)
                return __drawable_RGBA_to_RGB222_dither;
             if (palette_type == 3)
                return __drawable_RGBA_to_RGB221_dither;
             if (palette_type == 4)
                return __drawable_RGBA_to_RGB121_dither;
             if (palette_type == 5)
                return __drawable_RGBA_to_RGB111_dither;
             if (palette_type == 6)
                return __drawable_RGBA_to_RGB1_dither;
             if (palette_type == 7)
                return __drawable_RGBA_to_RGB666_dither;
          }
        else
          {
             if (palette_type == 0)
                return __drawable_RGBA_to_RGB332_fast;
             if (palette_type == 1)
                return __drawable_RGBA_to_RGB232_fast;
             if (palette_type == 2)
                return __drawable_RGBA_to_RGB222_fast;
             if (palette_type == 3)
                return __drawable_RGBA_to_RGB221_fast;
             if (palette_type == 4)
                return __drawable_RGBA_to_RGB121_fast;
             if (palette_type == 5)
                return __drawable_RGBA_to_RGB111_fast;
             if (palette_type == 6)
                return __drawable_RGBA_to_RGB1_fast;
             if (palette_type == 7)
                return __drawable_RGBA_to_RGB666_fast;
          }
     }
   return NULL;
}

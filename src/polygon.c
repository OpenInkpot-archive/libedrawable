...
DrawablePoly
__drawable_polygon_new()
{
   DrawablePoly           poly;

   poly = malloc(sizeof(_DrawablePoly));
   if (!poly)
      return NULL;
   memset(poly, 0, sizeof(_DrawablePoly));
   return poly;
}

void
__drawable_polygon_add_point(DrawablePoly poly, int x, int y)
{
   if (!poly->points)
     {
        poly->points = (DrawablePoint *) malloc(sizeof(DrawablePoint));
        if (!poly->points)
           return;
   ...
   else
     {
        poly->pointcount++;
        poly->points =
           (DrawablePoint *) realloc(poly->points,
                                  (poly->pointcount * sizeof(DrawablePoint)));
...
#define __drawable_point_inside_segment(p_x, p_y, s1_x, s1_y, s2_x,s2_y) \
(s1_y != s2_y) ? (p_y <= MAX(s1_y, s2_y) && p_y >= MIN(s1_y, s2_y)) : (p_x <= MAX(s1_x, s2_x) && p_x >= MIN(s1_x, s2_x))

#define __drawable_point_on_segment(p_x, p_y, s1_x, s1_y, s2_x, s2_y) \
__drawable_segments_intersect(p_x, p_y, p_x, p_y, s1_x, s1_y, s2_x, s2_y)

void
__drawable_polygon_get_bounds(DrawablePoly poly, int *px1, int *py1, int *px2,
                           int *py2)
{
...
static double
__drawable_point_delta_from_line(int p_x, int p_y, int s1_x, int s1_y, int s2_x,
                              int s2_y)
{
   if (s2_x - s1_x == 0.0)
      return p_x - s1_x;
...
__drawable_segments_intersect(int r1_x, int r1_y, int r2_x, int r2_y, int s1_x,
                           int s1_y, int s2_x, int s2_y)
{
   double              testS1R =
      __drawable_point_delta_from_line(s1_x, s1_y, r1_x, r1_y, r2_x, r2_y);
   double              testS2R =
      __drawable_point_delta_from_line(s2_x, s2_y, r1_x, r1_y, r2_x, r2_y);
   double              testR1S =
      __drawable_point_delta_from_line(r1_x, r1_y, s1_x, s1_y, s2_x, s2_y);
   double              testR2S =
      __drawable_point_delta_from_line(r2_x, r2_y, s1_x, s1_y, s2_x, s2_y);

   /* check if segments are collinear */
   if (testS1R == 0.0 && testS2R == 0.0)
     {
        if (__drawable_point_inside_segment(s1_x, s1_y, r1_x, r1_y, r2_x, r2_y)
            || __drawable_point_inside_segment(s2_x, s2_y, r1_x, r1_y, r2_x, r2_y)
            || __drawable_point_inside_segment(r1_x, r1_y, s1_x, s1_y, s2_x, s2_y)
            || __drawable_point_inside_segment(r2_x, r2_y, s1_x, s1_y, s2_x, s2_y))
           return TRUE;
        else
           return FALSE;
     }
...
unsigned char
__drawable_polygon_contains_point(DrawablePoly poly, int x, int y)
{
   int                 count = 0;
   int                 start = 0;
   ...
   for (n = 0; n < poly->pointcount; n++)
     {
        ...
        if (__drawable_point_on_segment(x, y, curr_x, curr_y, next_x, next_y))
           return TRUE;

        /* ignore horizontal segments from this point on */
        if (poly->points[cx].y != poly->points[nx].y)
          {
             if (__drawable_segments_intersect
                 (curr_x, curr_y, next_x, next_y, x, y, out_x, out_y))
               {
                  count++;

                  if (__drawable_point_on_segment
...
#define INIT_POLY()		\
do { \
   sfunc = __drawable_GetShapedSpanDrawFunction(op, dst_alpha, blend);	\
   if (!sfunc)  return;							\
									\
...
static void
__drawable_Polygon_DrawToData(DrawablePoly poly, char close, DATA32 color,
                           DATA32 * dst, int dstw,
                           int clx, int cly, int clw, int clh,
                           DrawableOp op, char dst_alpha, char blend)
{
   DrawableShapedSpanDrawFunction sfunc;
   IndexedValue       *ysort;
   PolyEdge           *edge;
   int                 k, a_a = 0;
...
static void
__drawable_Polygon_DrawToData_AA(DrawablePoly poly, char close, DATA32 color,
                              DATA32 * dst, int dstw,
                              int clx, int cly, int clw, int clh,
                              DrawableOp op, char dst_alpha, char blend)
{
...
void
__drawable_Polygon_DrawToImage(DrawablePoly poly, char close, DATA32 color,
                            ...
                            DrawableOp op, char blend, char anti_alias)
{
   ...
   if (poly->pointcount == 1)
     {
        (void)__drawable_Point_DrawToImage(poly->points[0].x, poly->points[0].y,
   ...
   if (poly->pointcount == 2)
     {
        (void)__drawable_Line_DrawToImage(poly->points[0].x, poly->points[0].y,
   ...
   if (anti_alias)
      __drawable_Polygon_DrawToData_AA(poly, close, color,
                                    im->data, im->w,
                                    clx, cly, clw, clh,
                                    op, IMAGE_HAS_ALPHA(im), blend);
   else
      __drawable_Polygon_DrawToData(poly, close, color,
                                 im->data, im->w,
                                 clx, cly, clw, clh,
                                 op, IMAGE_HAS_ALPHA(im), blend);
}
...
static void
__drawable_Polygon_FillToData(DrawablePoly poly, DATA32 color,
                           DATA32 * dst, int dstw,
                           int clx, int cly, int clw, int clh,
                           DrawableOp op, char dst_alpha, char blend)
{
...
static void
__drawable_Polygon_FillToData_AA(DrawablePoly poly, DATA32 color,
                              DATA32 * dst, int dstw,
                              int clx, int cly, int clw, int clh,
                              DrawableOp op, char dst_alpha, char blend)
{
                            ...
                            DrawableOp op, char blend, char anti_alias)
{
   ...
   if (anti_alias)
      __drawable_Polygon_FillToData_AA(poly, color,
                                    im->data, im->w,
                                    clx, cly, clw, clh,
                                    op, IMAGE_HAS_ALPHA(im), blend);
   else
      __drawable_Polygon_FillToData(poly, color,
                                 im->data, im->w,
                                 clx, cly, clw, clh,
                                 op, IMAGE_HAS_ALPHA(im), blend);
}

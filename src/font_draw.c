...
Drawable_Font_Glyph   *
drawable_font_cache_glyph_get(DrawableFont * fn, FT_UInt index)
{
   ...
   fg = drawable_hash_find(fn->glyphs, key);
   if (fg)
      return fg;
   ...
   fg = malloc(sizeof(struct _Drawable_Font_Glyph));
   if (!fg)
      return NULL;
   memset(fg, 0, (sizeof(struct _Drawable_Font_Glyph)));
   ...
   fn->glyphs = drawable_hash_add(fn->glyphs, key, fg);
   return fg;
}

void
drawable_render_str(DrawableImage * im, DrawableFont * fn, int drx, int dry,
                 const char *text, DATA8 r, DATA8 g, DATA8 b, DATA8 a,
                 char dir, double angle, int *retw, int *reth, int blur,
                 int *nextx, int *nexty, DrawableOp op, int clx, int cly,
                 int clw, int clh)
{
   int                 w, h, ascent;
   DrawableImage         *im2;
   ...
   drawable_font_query_advance(fn, text, &w, NULL);
   h = drawable_font_max_ascent_get(fn) - drawable_font_max_descent_get(fn);
   ...
   im2 = __drawable_CreateImage(w, h, data);
   if (!im2)
     {
        free(data);
        return;
   ...
   ascent = drawable_font_max_ascent_get(fn);

   drawable_font_draw(im2, col, fn, 0, ascent, text, &nx, &ny, 0, 0, w, h);

   /* OK, now we have small DrawableImage with text rendered, 
    * have to blend it on im */

   if (blur > 0)
      __drawable_BlurImage(im2, blur);
   ...
   switch (dir)
     ...
     case 1:                   /* to left */
        angle = 0.0;
        __drawable_FlipImageBoth(im2);
        break;
     case 2:                   /* to down */
        angle = 0.0;
        __drawable_FlipImageDiagonal(im2, 1);
        break;
     case 3:                   /* to up */
        angle = 0.0;
        __drawable_FlipImageDiagonal(im2, 2);
        break;
     default:
        break;
     }
   if (angle == 0.0)
     {
        __drawable_BlendImageToImage(im2, im, 0, 1, IMAGE_HAS_ALPHA(im), 0, 0,
   ...
   else
     {
        ...
        __drawable_BlendImageToImageSkewed(im2, im, 1, 1, IMAGE_HAS_ALPHA(im), 0,
                                        0, im2->w, im2->h, xx, yy, (w * ca),
                                        (w * sa), 0, 0, NULL, op, clx, cly, clw,
                                        clh);
     }

   __drawable_FreeImage(im2);
 ...
 */
DrawableFont          *
drawable_font_find_glyph(DrawableFont * first_fn, int gl, unsigned int *ret_index)
{
   DrawableFont          *fn = first_fn;
...
void
drawable_font_draw(DrawableImage * dst, DATA32 col, DrawableFont * fn, int x, int y,
                ...
                int clw, int clh)
{
   ...
   for (chr = 0; text[chr];)
     {
        FT_UInt             index;
        Drawable_Font_Glyph   *fg;
        DrawableFont          *fn_in_chain;
        int                 chr_x, chr_y;
        int                 gl;

        gl = drawable_font_utf8_get_next((unsigned char *)text, &chr);
        if (gl == 0)
           break;
        fn_in_chain = drawable_font_find_glyph(fn, gl, &index);
        ...
        fg = drawable_font_cache_glyph_get(fn_in_chain, index);
        if (!fg)
           continue;
   ...
   if (nextx)
      *nextx = (pen_x >> 8) - x;
   if (nexty)
      *nexty = drawable_font_get_line_advance(fn);
}
